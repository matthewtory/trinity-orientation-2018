...
class CacheManager {
  ...
  static Map<String, CacheManager> _instances = {};
  static Future<CacheManager> getInstance(String bucket) async {
    if (_instances[bucket] == null) {
      await synchronized(_lock, () async {
        if (_instances[bucket] == null) {
          CacheManager cacheManager = new CacheManager._(bucket);
          await cacheManager._init();

          _instances[bucket] = cacheManager;
        }
      });
    }


    return _instances[bucket];
  }

  CacheManager._(String bucket): this._firebaseStorage = new FirebaseStorage(storageBucket: bucket);
  ...
  Future<CacheObject> _downloadFile(
    ...
    StorageMetadata metadata = await _firebaseStorage.ref().child(url).getMetadata();
    await newCache.setDataFromHeaders({
    ...
    await _firebaseStorage.ref().child(url).writeToFile(new File(filePath)).future;
